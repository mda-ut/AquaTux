...
bool bmp_24_merged_read ( ifstream &file_in, unsigned long int width, long int height,
                          unsigned char *array)
{
   char c;
   bool error;
...
//
   ...
   for ( j = 0; j < height; j++ )
   {
      for ( i = 0; i < width; i++ )
      {
         ...
         array[j*width*3+i*3+2] = ( unsigned char ) c;
         numbyte = numbyte + 1;
         ...
         array[j*width*3+i*3+1] = ( unsigned char ) c;
         numbyte = numbyte + 1;
         ...
         array[j*width*3+i*3+0] = ( unsigned char ) c;
         numbyte = numbyte + 1;
      }
//
//  If necessary, read a few padding characters.
...
//
   ...
   array = new unsigned char[numbytes*3];
   /*
     for(int i=0; i<*height; i++)
     {
     array[i] = new unsigned char*[(int)(*width)];
     for(int j=0; j<*width; j++)
     {
     array[i][j] = new unsigned char[3];
     }
     } */
...
//
   ...
   {
      error = bmp_24_merged_read ( file_in, *width, *height, array );
...
//  Write the data.
//
   bmp_24_data_merged_write ( file_out, width, height, array );
//
//  Close the file.
//
   file_out.close ( );
